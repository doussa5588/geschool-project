...
function initializeEnrollmentChart() {
    ...
    @if(isset($charts['enrollment_evolution']) && $charts['enrollment_evolution']->count() > 0)
        try {
            ...
            console.warn('Erreur dans les données d\'inscription, utilisation des données par défaut');
        }
    @endif
...
function initializeLevelChart() {
    ...
    @if(isset($charts['students_by_level']) && $charts['students_by_level']->count() > 0)
        try {
            ...
            console.warn('Erreur dans les données de niveau, utilisation des données par défaut');
        }
    @endif
...
function initializeAttendanceChart() {
    ...
    @if(isset($attendanceStats['attendance_trend']) && $attendanceStats['attendance_trend']->count() > 0)
        try {
            ...
            console.warn('Erreur dans les données de présence, utilisation des données par défaut');
        }
    @endif
...
function initializePeriodHandlers() {
    ...
    periodInputs.forEach(input => {
        input.addEventListener('change', function() {
            ...
            // TODO: Recharger le graphique avec les nouvelles données
...
function updateEnrollmentChart(period) {
    // Cette fonction pourrait faire un appel AJAX pour récupérer de nouvelles données
    console.log('Mise à jour du graphique pour la période:', period);
    
    // Pour l'instant, on simule un changement de données
    if (enrollmentChart) {
...
function refreshDashboard() {
    ...
    // Simulation d'actualisation (remplacer par un vrai appel AJAX)
    if (refreshBtn) {
    ...
    setTimeout(() => {
        ...
        updateStatsCards({
            total_students: Math.floor(Math.random() * 1000) + 500,
            total_teachers: Math.floor(Math.random() * 100) + 50,
            total_classes: Math.floor(Math.random() * 50) + 20,
            students_growth: Math.floor(Math.random() * 20) - 10,
            teachers_growth: Math.floor(Math.random() * 10) - 5
        });
